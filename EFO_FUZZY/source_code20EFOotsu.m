...
for k=1:3
    E(k,:) = imhist(I(:,:,k));
%     E(k,:) = E(k,:)/sum(E(k,:));
    [Entp(k,:),threshlevels(k,:),convplot(k,:)] = EFOsc20otsu(N_var,N_emp,Max_gen,minval,maxval,R_rate,Ps_rate,P_field,N_field,E(k,:),meanthresh(k));       %%% threshold levels and minimum Entropy obtained
