...
Number_of_levels = 7;       %%% discrete values
...
for i=1:3
    meanthresh(i) = mean2(sgrays(:,:,i)); 
end
n = 15;
nub = Number_of_levels;
lowerbounds = 0.*ones(1,256);
upperbounds = 255.*ones(1,256);
maxiter = 2000;

for k=1:3
    E(k,:) = imhist(I(:,:,k));
%     E(k,:) = E(k,:)/sum(E(k,:));
    [Entp(k,:),threshlevels(k,:),convplot(k,:)] = bsasc20otsu(n,nub,1,lowerbounds,upperbounds,maxiter,E(k,:),meanthresh(k));       %%% threshold levels and minimum Entropy obtained
