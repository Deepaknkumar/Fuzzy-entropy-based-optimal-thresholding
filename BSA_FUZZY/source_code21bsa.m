...
Number_of_levels = 4;       %%% discrete, valid for 2,5 only
...
for k=1:3
    E(k,:) = imhist(I(:,:,k));
%     E(k,:) = E(k,:)/sum(E(k,:));
    [Entp(k,:),values(k,:),convplot(k,:)] = bsasc20(n,nub,1,lowerbounds,upperbounds,maxiter,E(k,:));       %%% threshold levels and minimum Entropy obtained
