...
Number_of_levels = 2;       %%% Input Number of Levels Required(Currently valid for two only)
...
for k=1:3
    E(k,:) = imhist(I(:,:,k));
%     E(k,:) = E(k,:)/sum(E(k,:));
    [Entp(k,:),values(k,:),convplot(k,:)] = abcsc20(n,Number_of_levels,E(k,:));       %%% threshold levels and minimum Entropy obtained
