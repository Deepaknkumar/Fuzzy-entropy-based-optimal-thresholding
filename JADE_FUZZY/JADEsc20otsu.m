    function [outcome,u,fminval] = JADEsc20otsu(popsize,nub,E,muT)
    Maxiter = 2000;
    time = 1;
   iter = 1;
    ...
    %popsize = 10;
     lu = [0.*ones(1,n);255.*ones(1,n)];
        ...
        for i=1:popsize
            valParents(i,:) = fitnessfuncsc20otsu(popold(i,:),E,muT);
        end
        ...
        while FES  < n * Maxiter %& min(fit)>error_value(problem)
            ...
            for inb=1:popsize
                valOffspring(inb,:) = fitnessfuncsc20otsu(popold(inb,:),E,muT);
            end
            FES = FES + popsize;
