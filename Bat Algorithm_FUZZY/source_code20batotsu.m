...
for i=1:3
    meanThreshs(i) = mean2(sgrays(:,:,i));
end

n = 15;
Maxiter = 2000;
params = [n Maxiter .5 .5];
for k=1:3
    E(k,:) = imhist(I(:,:,k));
%     E(k,:) = E(k,:)/sum(E(k,:));
    [Entp(k,:),threshlevels(k,:),convplot(k,:)] = bat_algorithmsc20otsu(params,Number_of_levels,E(k,:),meanThreshs(k));       %%% threshold levels and minimum Entropy obtained
